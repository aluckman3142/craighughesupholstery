<x-app-layout>
    ...
    <div class="m-5">
            <h1>Edit Before &amp; After Image {{$beforeAfter->title}}</h1>
            <hr>
        ...
        <form name="edit-before-after-form" id="edit-category-image-form" method="post" action="{{ URL::to('dashboard/before-after/'.$beforeAfter->id.'/update') }}">
            @csrf
            <div class="form-group">
                <label for="title">Title</label>
                <input type="text" id="title" name="title" class="form-control" value="{{  old('title', $beforeAfter->title) }}">
            </div>
            <div class="form-group">
                <label for="description">Description</label>
                <textarea id="description" name="description" class="form-control">{{ old('description', $beforeAfter->description) }}</textarea>
            </div>
            <div class="hidden">
                <label for="before_image_name">Before Image Name</label>
                <input type="text" id="before_image_name" name="before_image_name" hidden value="{{ old('before_image_name') }}">
            </div>
            <div class="hidden">
                <label for="after_image_name">After Image Name</label>
                <input type="text" id="after_image_name" name="after_image_name" hidden value="{{ old('after_image_name') }}">
            </div>
            <div class="form-group">
                <div class="grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div>
                        <div id="before-image-preview" class="float-right">
                            ...
                            @else
                                <img src="{{$beforeAfter->before_src}}" />
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div>
                        <div id="after-image-preview" class="float-right">
                            ...
                            @else
                                <img src="{{$beforeAfter->after_src}}" />
                            @endif
                        </div>
                    </div>
                </div>
            </div>
            <button type="submit" class="btn btn-success btn-sm uppercase">Update Before &amp; After Image</button>
        </form>
    </div>
